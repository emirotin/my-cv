...
const separateChars = (matrix) => {
	...
	for ({
		...
		for (let j = jmin; j <= jmax; j++) {
			const allWhite = j == jmax || !_.some(sliceHeightInterval.map(i => matrix[i][j]))
			if (state === 0 && !allWhite) {
				state = 1
				charStart = j
			} else if (state === 1 && allWhite) {
				...
				chars.push(_.range(charVertStart, charVertEnd + 1).map(i => matrix[i].slice(charStart, j)))
			}
		}
	}
...
const mapChars = (chars, charMapping) => {
	...
	for (const def of charMapping) {
		let { vOffset, chars: cs } = typeof def === 'string'
			? { vOffset: 0, chars: def }
			: def
...
const buildFont = (pngFilePath, charMapping, config) =>
	parseFile(pngFilePath, charMapping)
	.then(charDefs => Object.assign({}, config, { characterMap: charDefs }))

// ---

PNG_FILE = path.resolve(__dirname, '..', 'lcd-font', 'alphabet.png')
CHAR_MAPPING = require('../lcd-font/charmap.json')
CONFIG = require('../lcd-font/config.json')
OUT_FILE = path.resolve(__dirname, '..', 'lcd-font', 'alphabet.json')

buildFont(PNG_FILE, CHAR_MAPPING, CONFIG)
.then(fontConfig => {
